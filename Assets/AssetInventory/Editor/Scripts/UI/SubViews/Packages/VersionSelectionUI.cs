        ...
        public override void OnGUI(Rect rect)
        {
            ...
            if (_packageInfo == null)
            {
                if (_info.PackageSource == PackageSource.Git)
                {
                    ...
                    if (GUILayout.Button(CommonUIStyles.Content($"Install Indexed {_info.GetVersion()}")))
                    {
                ...
                else
                {
                    ...
                    {
                        if (GUILayout.Button(CommonUIStyles.Content($"Install Indexed {_info.LatestVersion}")))
                        {
                            _callback?.Invoke(_info.LatestVersion);
                            editorWindow.Close();
                        }
            ...
            foreach (string version in _packageInfo.versions.all.Reverse())
            {
                ...
                EditorGUI.BeginDisabledGroup((!compatible && !AI.ShowAdvanced()) || isCurrent);
                if (GUILayout.Button(CommonUIStyles.Content(version, "Install this version"), CommonUIStyles.wrappedButton, GUILayout.Width(150)))
                {
