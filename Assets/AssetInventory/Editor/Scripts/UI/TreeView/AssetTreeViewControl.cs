...
namespace AssetInventory
{
    internal sealed class AssetTreeViewControl : TreeViewWithTreeModel<AssetInfo>
    {
        ...
        private void CellGUI(Rect cellRect, TreeViewItem<AssetInfo> item, Columns column, ref RowGUIArgs args)
        {
            ...
            Texture checkmarkIcon = CommonUIStyles.IconContent("Valid", "d_Valid", "|Indexed").image;
            Rect rect = cellRect;
            ...
            switch (column)
            {
                ...
                case Columns.InternalState:
                    EditorGUI.LabelField(cellRect, StringUtils.CamelCaseToWords(item.Data.CurrentState.ToString()));
                    break;
                ...
                case Columns.Name:
                    ...
                    {
                        Texture folderIcon = CommonUIStyles.IconContent("Folder Icon", "d_Folder Icon").image;
                ...
                case Columns.State:
                    EditorGUI.LabelField(cellRect, item.Data.OfficialState.ToString());
                    break;
                ...
                case Columns.Tags:
                    if (item.Data.PackageTags.Count > 0)
                    {
                        ...
                        {
                            RenderColoredTags(cellRect, item.Data.PackageTags);
                        }
                        else
                        {
                ...
                case Columns.Version:
                    ...
                    bool indexingRequired = (item.Data.CurrentState == Asset.State.New || item.Data.CurrentState == Asset.State.InProcess)
                        && item.Data.IsDownloaded
                        && (item.Data.AssetSource != Asset.Source.RegistryPackage || AI.Actions.IndexPackageCache);
                    ...
                    if ((item.Data.AssetSource == Asset.Source.Archive || item.Data.AssetSource == Asset.Source.CustomPackage) && string.IsNullOrWhiteSpace(item.Data.GetVersion()))
                    {
                        if (AI.ShowAdvanced() && GUI.Button(rect, "enter manually"))
                        {
                            ...
                            PopupWindow.Show(BasicEditorUI.GetPopupPositionAtMouse(), textUI);
                        }
                    }
                    else
                    {
                        ...
                        if (updateAvailable || indexingRequired)
                        {
                            Vector2 size = EditorStyles.label.CalcSize(version);
                            float iconX = rect.x + Mathf.Min(size.x, cellRect.width - 32) + 2;
                            float iconY = rect.y + 1;
                            Rect iconRect = new Rect(iconX, iconY, 16, 16);

                            if (updateAvailable)
                            {
                                Texture updateIcon = CommonUIStyles.IconContent("preAudioLoopOff", "Update-Available", "|Update Available").image;
                                Color color = item.Data.AssetSource == Asset.Source.CustomPackage ? Color.gray : Color.white;
                                GUI.DrawTexture(iconRect, updateIcon, ScaleMode.StretchToFill, true, 0, color, Vector4.zero, Vector4.zero);
                                iconRect.x += 18;
                            }

                            if (indexingRequired)
                            {
                                Texture indexIcon = CommonUIStyles.IconContent("d_Refresh", "d_Refresh", "|Indexing Required").image;
                                GUI.DrawTexture(iconRect, indexIcon, ScaleMode.StretchToFill, true, 0, Color.gray, Vector4.zero, Vector4.zero);
                            }
                        }
                    }
                    break;
        ...
        private static void RenderColoredTags(Rect cellRect, IList<TagInfo> tags)
        {
            ...
            {
                float remainingWidth = cellRect.xMax - currentX;
                if (remainingWidth < 20f) break;

                TagInfo tagInfo = tags[i];
                float tagWidth = UIStyles.CalcTagWidth(tagInfo.Name, UIStyles.TagStyle.Neutral, remainingWidth);
                Rect tagRect = new Rect(currentX, cellRect.y, tagWidth, cellRect.height);

                UIStyles.DrawTag(tagRect, tagInfo.Name, tagInfo.GetColor(), UIStyles.TagStyle.Neutral);
