...
namespace AssetInventory
{
    ...
    internal class TreeViewWithTreeModel<T> : TreeView where T : TreeElement
    {
        ...
        {
            if (id <= 0) return;
            if (OnContextMenuPopulate == null) return;

            List<int> selectedIds = new List<int>(GetSelection());

            // Use current selection if the clicked item is part of it; otherwise operate on the clicked item alone
            List<T> selection = new List<T>();
            if (selectedIds.Contains(id))
            {
                foreach (int sid in selectedIds)
                {
                    T data = TreeModel.Find(sid);
                    if (data != null) selection.Add(data);
                }
            }
            else
            {
                T clicked = TreeModel.Find(id);
                if (clicked != null) selection.Add(clicked);
            }

            GenericMenu menu = new GenericMenu();
            int clickedIndex = Math.Max(0, selection.FindIndex(a => a.TreeId == id));
            OnContextMenuPopulate.Invoke(menu, selection, clickedIndex);
            menu.ShowAsContext();
            Event.current?.Use();
        }
        ...
        protected override void ContextClicked()
        {
            ...
            GenericMenu menu = new GenericMenu();
            int clickedIndex = selection.Count > 0 ? 0 : -1;
            OnContextMenuPopulate.Invoke(menu, selection, clickedIndex);
            menu.ShowAsContext();
            Event.current?.Use();
        }
